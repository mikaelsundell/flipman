...
bool
Media::is_supported(const QString& extension) const
{
    plugins::PluginRegistry* registry = plugins::PluginRegistry::instance();
...
Media::read()
{
    Q_ASSERT("media is not open" && is_open());
...
Media::audio() const
{
    Q_ASSERT("media is not open and must support audio" && is_open() && p->d.reader->supports_audio());
...
Media::image() const
{
    Q_ASSERT("media is not open and must support images" && is_open() && p->d.reader->supports_image());
